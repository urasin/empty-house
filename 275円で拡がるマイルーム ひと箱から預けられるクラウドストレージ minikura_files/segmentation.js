    ...
    FOut.DMP = (function (undef) {
        ...
        function _run(target, request_count, sent_user) {
            ...
            for_each(target, function(q) {
                ...
                if (q.cohort && typeof document.interestCohort === 'function') {
                    document.interestCohort().then(function(cohort) {
                        queries.push('cohort_id=' + encodeURIComponent(cohort.id));
                        queries.push('cohort_version=' + encodeURIComponent(cohort.version));
                        (new Image()).src = host + '/' + user_id + '/cnt?' + queries.join('&');
                    }, function(error) {
                        queries.push('cohort_error=' + encodeURIComponent(error));
                        (new Image()).src = host + '/' + user_id + '/cnt?' + queries.join('&');
                    });
                } else {
                    (new Image()).src = host + '/' + user_id + '/cnt?' + queries.join('&');
